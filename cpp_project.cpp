...
class resource{
    public:
        ...
        void reduce(int n){
            free_resources-=n;
            cout<<"Total resources of "<<resource_name<<" are reduced to : "<<free_resources<<endl;
            if(free_resources<=0){
                cout<<"Processes are in unsafe mode!"<<endl;
                deadlock=true;
            }
        }
};  
      ...
      //logic for RR Priority remainning
        void assignProcess(core& receiver){
            ...
            if(receiverBatchName=="SJF"){
                ...
                if(alreadyWokring(receiver)){
                    cout<<receiver.coreName<<" is already working on a process that used resources"<<endl;
                    return;
                }else if(deadlock){
                    cout<<"Processes that require resources have cause the deadlock to occur"<<endl;
                    cout<<"Both core5 & core 6 are IDLE now!"<<endl;
                    receiver.workingProcessUsingResourcesPtr=nullptr;
                    return;
                }
                
                if(receiver.coreName=="core5"){
                        process_using_resource* temp=new process_using_resource(ProcessAllocation[0].top());
                        receiver.workingProcessUsingResourcesPtr=temp;
                } else if(receiver.coreName=="core6"){
                        process_using_resource* temp=new process_using_resource(ProcessAllocation[1].top());
                        receiver.workingProcessUsingResourcesPtr=temp;
                }
            }else {
                cout<<"Invalid"<<endl;
        ...
        while(file>>PId>>bt>>memory>>total>>allo){
           ...
           process_using_resource p(PId,bt,memory,total_need,allocated); 
            for(int i=0;i<all_resources.size();i++){
                all_resources[i].reduce(allocated[i]);
            }
